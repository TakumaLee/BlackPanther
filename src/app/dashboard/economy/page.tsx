...
import { Loader2, Save, RotateCcw, TrendingUp, DollarSign, Gift, Star } from 'lucide-react';
import { getEconomyConfig, updateEconomyConfig, resetEconomyConfig } from '@/lib/api/economy';

interface EconomyConfig {
  ai_analysis_pricing: Record<string, any>;
  daily_signin_rewards: any;
  article_extensions: Record<string, number>;
  translation_pricing: Record<string, number>;
  vip_membership: Record<string, number>;
  invite_rewards: Record<string, number>;
  new_user_rewards: any;
}
...
export default function EconomyPage() {
  ...
  const updateAIPrice = (model: string, field: string, value: any) => {
    if (!editedConfig) return;
    setEditedConfig({
      ...editedConfig,
      ai_analysis_pricing: {
        ...
        [model]: {
          ...
          [field]: field === 'cost' || field === 'original_cost' ? parseInt(value) : value
        }
      }
    });
  };
  ...
  const updateSigninReward = (field: string, value: any) => {
    if (!editedConfig) return;
    if (field === 'base_reward') {
      setEditedConfig({
        ...
        daily_signin_rewards: {
          ...
          base_reward: parseInt(value)
        }
      });
    } else {
      // Handle streak bonuses
      const [_, day] = field.split('_');
      setEditedConfig({
        ...editedConfig,
        daily_signin_rewards: {
          ...editedConfig.daily_signin_rewards,
        ...
        <TabsContent value="ai-analysis" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              {Object.entries(editedConfig.ai_analysis_pricing).map(([model, settings]: [string, any]) => (
